...
export default function HomeScreen() {
  ...
  const [lastSwiped, setLastSwiped] = useState<boolean>(false);
  ...
  const handleSwipe = (index: number, direction: "left" | "right") => {
    if (index >= profiles.length)
      return (
        <div className="flex-1 items-center text-white text-lg justify-center">
          {" "}
    ...
    if (direction === "left") {
      setLikes([...likes, profiles[index].id]);
    }
    setLastSwiped(true);
  ...
  return (
    <SafeAreaView className="flex-1 items-center justify-center bg-gray-900">
      ...
      <View className="absolute bottom-16 flex-row space-x-8">
        <TouchableOpacity
          className="bg-red-500 p-5 rounded-full shadow-lg scale-110 active:scale-100"
          onPress={() => handleSwipe(0, "left")}
        >
          <Ionicons name="close" size={32} color="white" />
        </TouchableOpacity>
        <TouchableOpacity
          className="bg-blue-500 p-5 rounded-full shadow-lg scale-110 active:scale-100"
          onPress={refreshSwipe}
        >
          <Ionicons name="refresh" size={32} color="white" />
        </TouchableOpacity>
        <TouchableOpacity
          className="bg-green-500 p-5 rounded-full shadow-lg scale-110 active:scale-100"
          onPress={() => handleSwipe(0, "right")}
        >
      ...
      <View className="absolute bottom-5">
        <Text className="text-gray-500 text-sm">
          {matches.length} matches trouvés ✅
        </Text>
      </View>
    </SafeAreaView>
  );
